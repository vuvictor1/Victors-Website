...
def contacts_page(): # Contacts page
    ...
    with ui.row().style('justify-content: center; align-items: center; width: 100%; height: 50vh;'): # Center the contacts title
        ...
        with ui.column().classes('about-me').style('max-width: 600px;'): # Adjust padding and column width
            ui.label('Contact Me').style('font-size: 32px; color: white; text-align: center;') # Mail title
...
def submit_form(name, email, message): # Submit form
    if name and email and message: # Check if fields are not empty   
        ui.notify('Thank you. Your message has been sent.', position='center', color='green') # notify message
    else: 
        ui.notify('Please fill in all fields.', position='center', color='red') # notify error
