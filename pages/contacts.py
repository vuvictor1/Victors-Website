...
def contacts_page(): # Contacts page
    ...
    with ui.row().classes('justify-center w-full my-4'): # Center the contacts title
        ...
        with ui.column().style('max-width: 50rem;').classes('items-center text-center p-5 bg-gray-800 rounded-lg shadow-lg'):
            ...
            ui.html(f'''
                <form action="https://api.web3forms.com/submit" method="POST" style="width: 100%;">
                    ...
                    <input type="email" name="email" placeholder="Your Email (So I can reply)" required style="font-size: 16px; width: 100%; margin-bottom: 10px; background-color: #3B3B3B; color: white; border: none; padding: 10px; border-radius: 5px;">
                    ...
                    <div style="text-align: center;">
                        <button type="submit" style="color: white; width: 25%; background-color: #5898D4; padding: 10px 20px; cursor: pointer; border-radius: 5px; font-size: 20px;">Send Mail</button>
                    </div>
                </form>
            ''')
